...
class AnalysisService:
    ...
    async def process_batch_job(
        ...
        """
        Background worker processing CSV rows and writing real-time progress.
        """
            ...
            try:
                ...
                for idx, row in enumerate(rows):
                    ...
                    try:
                        ...
                        logger.error(f"Error processing row {idx} in job {job_id}: {e}")
                        failed += 1

                    # Commit periodically for live SSE updates
                    result = await session.execute(select(AnalysisJob).filter_by(id=job_id))
